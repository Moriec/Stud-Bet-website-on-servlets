...
create table betting_events(
	...
	semester int,
	academic_year varchar(10),
	event_type varchar(20), -- EXAM, ASSIGNMENT, FINAL
...
create table bets(
	...
	payout_amount int check(payout_amount > 0),
	created_at timestamp default current_timestamp,
	resolved_at timestamp,
	predicted_score_min int not null check(predicted_score_min >= 0),
	predicted_score_max int not null check(predicted_score_max <= 100),
	check(predicted_score_min <= predicted_score_max)
);

create table transactions(
	transaction_id serial primary key,
	user_id int references users(user_id),
	transaction_type varchar(20) not null, -- DEPOSIT, WITHDRAW, BET_PLACED, BET_WIN, BET_LOSS
	amount int,
	balance_before int,
	balance_after int,
...
create table achievements(
	...
	achievement_type varchar(20), -- BETTING, ACCURACY, SOCIAL
	criteria_json text
);
...
ALTER TABLE betting_events ADD COLUMN IF NOT EXISTS description TEXT;
